package com.example.barcodeReader.database
...
interface InventoryItemOfflineModeDao {
    ...
    @Query("select * from InventoryItemOfflineMode")
    fun getTableCount(): LiveData<List<InventoryItemOfflineMode>>
}
...
@Database(
    entities = [User::class, SavedUsers::class, InventoryItem::class, InventoryItemOfflineMode::class],
    version = 1,
    exportSchema = false
)
abstract class TopSoftwareDatabase : RoomDatabase() {
    ...
    abstract val inventoryItemOfflineModeDao: InventoryItemOfflineModeDao
